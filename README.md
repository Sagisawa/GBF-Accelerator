...
- **前台素材优先调度与 QoS 避让 (v1.7.2)**：
  - 动态监测前台活动请求，当前台拉取战斗与首屏素材时，后台预加载任务主动暂停让道；
  - 预加载素材扫描从前台请求路径解耦至后台队列，将磁盘检查卸载至工作线程池，降低主事件循环阻塞风险；
  - 启动阶段内存预热调整至 1500 项，启动阶段磁盘扫描时间由 ~7.5 秒缩短至 1.5 ~ 2.5 秒。
...
- **纯净响应与私有头剥离**：向浏览器交付的所有静态素材与动态 API 响应中，不注入任何 `X-Proxy-*` 私有调试头，避免引入非必要的特征标头。
